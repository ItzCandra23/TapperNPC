...
(function (TapperUI) {
    function manage(entity, player, callback) {
        ...
        form.show(player).then((res) => {
            if (res.selection === undefined)
                return;
            if (res.selection === 0)
                return Actions.main(entity, player, () => manage(entity, player, callback));
            if (res.selection === 1)
                return editNameTag(entity, player, () => manage(entity, player, callback));
            if (res.selection === 2)
                return manageEquipment(entity, player, () => manage(entity, player, callback));
            if (res.selection === 3)
            ...
            if (res.selection === 7)
                return setRotation(entity, player, () => manage(entity, player, callback));
            if (res.selection === 8)
                return deleteNPC(entity, player, () => manage(entity, player, callback));
            if (res.selection === 9 && callback)
                return callback();
        });
    }
    TapperUI.manage = manage;
    async function deleteNPC(entity, player, callback) {
        ...
        form.show(player).then((res) => {
            ...
            if (res.selection === 1 && callback)
                callback();
        });
    }
    TapperUI.deleteNPC = deleteNPC;
    async function setRotation(entity, player, callback) {
        ...
        form.show(player).then((res) => {
            if (res.formValues === undefined)
                return callback && callback();
            const value = res.formValues[0] ?? 0;
            try {
    ...
    TapperUI.setRotation = setRotation;
    async function manageEquipment(entity, player, callback) {
    ...
    TapperUI.manageEquipment = manageEquipment;
    async function setEquipment(entity, slot, player, callback) {
    ...
    TapperUI.setEquipment = setEquipment;
    ...
    let Actions;
    (function (Actions) {
        function main(entity, player, callback) {
            const actions = TapperNPC.getActions(entity);
            const form = new ActionFormData();
            ...
            form.button(`§8[ §c${callback ? `BACK` : `CLOSE`} §8]`, "textures/blocks/barrier");
            form.show(player).then((res) => {
                if (res.selection === undefined || res.selection === actions.length + 1)
                    return callback && callback();
                if (res.selection === actions.length)
                    return addAction(entity, player, () => main(entity, player, callback));
        ...
        async function ActionForm(player, options) {
            ...
            try {
                ...
                const command = result.formValues[1];
                const conditional = result.formValues.length === 3 ? result.formValues[2] : false;
                return {
                    actor,
                    command,
                    conditional
        ...
        Actions.ActionForm = ActionForm;
        async function addAction(entity, player, callback) {
            try {
        ...
        function manageAction(entity, player, targetIndex, callback) {
            ...
            form.show(player).then((res) => {
                if (res.selection === 0)
                    return editAction(entity, player, targetIndex, () => manageAction(entity, player, targetIndex, callback));
                if (res.selection === 1) {
                    try {
                ...
                if (res.selection === 3)
                    return deleteAction(entity, player, targetIndex, callback);
                if (res.selection === 4 && callback)
                    return callback();
            });
        }
        Actions.manageAction = manageAction;
        async function editAction(entity, player, targetIndex, callback) {
            try {
                const action = TapperNPC.getAction(entity, targetIndex);
                if (!action)
                    return callback && callback();
        ...
        async function deleteAction(entity, player, targetIndex, callback) {
            const action = TapperNPC.getAction(entity, targetIndex);
            if (!action)
                return callback && callback();
            const form = new ActionFormData();
    ...
    })(Actions = TapperUI.Actions || (TapperUI.Actions = {}));
})(TapperUI || (TapperUI = {}));
